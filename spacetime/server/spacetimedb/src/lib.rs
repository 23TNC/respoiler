...
fn validate_nonzero_id(id_name: &str, id: u64) -> Result<(), String> {
  if id == 0 {
...
fn validate_not_self_owned_card(card_id: u64, owner_card_id: u64) -> Result<(), String> {
...
#[reducer]
pub fn create_player(ctx: &ReducerContext, player_id: u64, card_id: u64) -> Result<(), String> {
...
pub fn update_player_card(
  ...
  ctx.db.player().player_id().update(Player { card_id, ..existing });
  Ok(())
}
...
#[reducer]
pub fn delete_soul_alignment(ctx: &ReducerContext, card_id: u64) -> Result<(), String> {
  validate_nonzero_id("card_id", card_id)?;
...
pub fn update_tile(
  ...
  let existing = ctx.db.tile().tile_id().find(&tile_id).ok_or("tile not found")?;
...
// Deletes a tile row.
#[reducer]
pub fn delete_tile(ctx: &ReducerContext, tile_id: u64) -> Result<(), String> {
  validate_nonzero_id("tile_id", tile_id)?;
...
#[reducer]
pub fn delete_tile_tracker(ctx: &ReducerContext, tile_id: u64) -> Result<(), String> {
  validate_nonzero_id("tile_id", tile_id)?;
...
#[reducer]
pub fn delete_event_tracker(ctx: &ReducerContext, tile_id: u64) -> Result<(), String> {
  validate_nonzero_id("tile_id", tile_id)?;
...
#[reducer]
pub fn delete_slot_tracker(ctx: &ReducerContext, tile_id: u64) -> Result<(), String> {
  validate_nonzero_id("tile_id", tile_id)?;
...
pub fn update_card(
  ...
  let existing = ctx.db.card().card_id().find(&card_id).ok_or("card not found")?;
...
// Deletes a card row.
#[reducer]
pub fn delete_card(ctx: &ReducerContext, card_id: u64) -> Result<(), String> {
  validate_nonzero_id("card_id", card_id)?;
...
pub fn update_card_position_lock(
  ...
  ctx.db
    .card_tracker()
    .card_id()
    .update(CardTracker { position_lock, ..existing });
  Ok(())
}
...
pub fn update_card_position_hold(
  ...
  ctx.db
    .card_tracker()
    .card_id()
    .update(CardTracker { position_hold, ..existing });
  Ok(())
}

// Deletes a card tracker row.
#[reducer]
pub fn delete_card_tracker(ctx: &ReducerContext, card_id: u64) -> Result<(), String> {
  validate_nonzero_id("card_id", card_id)?;
...
#[reducer]
pub fn delete_action_tracker(ctx: &ReducerContext, card_id: u64) -> Result<(), String> {
  validate_nonzero_id("card_id", card_id)?;
